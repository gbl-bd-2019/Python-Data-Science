...
-- выводит состав суши c bvtyfvb ceib
select
...
-- Выбираем выбор пользователя из роллов	
...
-- Выбираем количество выбранных роллов в сетах	
...
-- Выбираем общее количество роллов в сетах	
	create or replace view sets_count (st_count, set_id) as
		select count(*), set_id
			FROM tbl_set_contents
			group by set_id;

-- Объединяем таблицы с общим и выбранным количеством	
...
-- Таблица с отношением выбранных к общему в сетах, сортированное по отношению	
...
-- Возвращаем первое максимальное отношение
